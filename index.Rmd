---
title: "Tarea Programada Covid-19"
...
### Mapa de Covid-19 según la distribución por cantones en Costa Rica

```{r}
m <- leaflet(infomapa_covid19cr_cantones)
pal <- colorBin("Spectral", infomapa_covid19cr_cantones$confirmados, bins =7, pretty = TRUE, na.color = "White")
m %>% addPolygons(
       ...
       addLegend(pal = pal, values = ~confirmados, opacity = 0.6, 
                 title = "Cantidad casos confirmados",
                 position = "bottomright")
```

### Mapa de Covid-19 según la distribución por provincia en Costa Rica
...
mns <- infomapa_covid19cr_cantones %>%
       ...
       fy %>% addPolygons(
              ...
              addLegend(pal = pal, 
                values = ~confirmados, 
                opacity = 0.6, 
                title = "Cantidad de casos confirmados", 
                position = "bottomright")
``` 

### Gráfico de cantidad de Confirmados por provincia de Covid-19 en Costa Rica  
...
-----------------------------------------------------------------------
### Tabla de Confirmados, Fallecidos y Recuperados por cantones de Costa Rica 
...
valueBox(confirmadospaises, icon = "", color="orange")
```
...
valueBox(activospaises, icon = "fa-ba", color="red")
```
...
valueBox(fallecidospaises, icon = "", color="grey")
```
...
valueBox(recuperadospais, icon = "", color="green")
```